...
int statement(Parser *parser)
{
    ...
    if(parser->curr_token.type == TOKEN_IDENTIFIER)
    {
        ...
        CHECK_ERROR();
        if((parser->symbol_data != NULL) && (parser->symbol_data->symbol_validity == SYMBOL_BUILT_IN)) // if the ID already exists in our GTS and it's a built in function ID then call syntax_error
        {
            return SYNTAX_ERROR; 
        }
